...
/* calcul déclaration */
	...
	if(reste_p != 0) {
		...
		x = reste_r;
		t1_r = coutMoinReste(t1_r, &reste_r);
		p_to_r_t1 += (x - reste_r) / 15;
		x = reste_r;
		t2_r = coutMoinReste(t2_r, &reste_r);
		p_to_r_t2 += (x - reste_r) / 15;
		x = reste_r;
		t3_r = coutMoinReste(t3_r, &reste_r);
		p_to_r_t3 += (x - reste_r) / 15;
		x = reste_r;
		t4_r = coutMoinReste(t4_r, &reste_r);
		p_to_r_t4 += (x - reste_r) / 15;
		x = reste_r;
	}

	
	if(reste_r != 0 || reste_r >= 15) {
		reste_m += reste_r / 15;
		x = reste_m;
		t1_m = coutMoinReste(t1_m, &reste_m);
		r_to_m_t1 += (x - reste_m);
...
void m_to_r(int *t_r, int *t_m, int gain_r, int augment, int matrix_by_day, int *r_to_m_t) {
	...
	if(*t_m != 0) {
		...
		if(r_time_overload_t * matrix_by_day > 0) {
			*r_to_m_t += r_time_overload_t * matrix_by_day;
			*t_r += r_time_overload_t * matrix_by_day * 15;
			*t_m -= r_time_overload_t * matrix_by_day;
		}
	}
}
